const { proto, delay, getContentType } = require('@onedevil405/baileys')
...
/**
 * RequestQueue - Throttles API requests to prevent rate limiting
 * Maintains separate queues for different request types
 */
class RequestQueue {
  constructor(delayMs = 2500) {
    this.queues = new Map(); // Queue per group/jid
    this.delayMs = delayMs;
    this.processing = new Map();
  }

  async add(queueKey, fn, priority = 0) {
    if (!this.queues.has(queueKey)) {
      this.queues.set(queueKey, []);
    }
    
    return new Promise((resolve, reject) => {
      this.queues.get(queueKey).push({ fn, resolve, reject, priority, timestamp: Date.now() });
      this.process(queueKey);
    });
  }

  async process(queueKey) {
 ...
 */
async function executeWithBackoff(fn, maxRetries = 4, baseDelayMs = 3000) {
  let lastError;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit && attempt < maxRetries - 1) {
        ...
        console.log(`⏱️  Rate limited. Waiting ${delayMs}ms before retry (attempt ${attempt + 1}/${maxRetries - 1})...`);
