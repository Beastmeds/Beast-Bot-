import { WAMessageStubType } from '@onedevil405/baileys'
import PhoneNumber from 'awesome-phonenumber'
import chalk from 'chalk'
import { watchFile } from 'fs'

let terminalImage = ''
try { terminalImage = global.opts['img'] ? (await import('terminal-image')).default : '' } catch (e) {}
...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(`
${chalk.redBright('%s')} ${chalk.black(chalk.bgBlack('%s'))} ${chalk.black(chalk.bgGreen('%s'))} ${chalk.magenta('%s [%s %sB]')}
${chalk.magenta('%s')} ${chalk.redBright('%s%s')} ${chalk.bgGreen(`\nin der gruppe`)} ${chalk.green('%s')} ${chalk.black(chalk.bgRed('%s'))}
`.trim(),
    me + ' ~' + conn.user.name,
    (m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toLocaleString('de-De', { timeZone: 'CET' }),
    ...
    user ? ', UsrXp ' + user.exp + ', Geld ' + user.dolares + ', Limit ' + user.limit : ', Level ' + (user?.level || '?'),
    m.chat + (chat ? ' ~' + chat : ''),
