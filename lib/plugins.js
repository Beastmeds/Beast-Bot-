...
async function filesInit(pluginFolder = undefined, pluginFilter = undefined, conn) {
    if (!pluginFolder) pluginFolder = global.__dirname ? Helper.__dirname(join(global.__dirname, '../plugins/index')) : resolve('plugins/index')
    if (!pluginFilter) pluginFilter = filename => /\.(mc)?js$/.test(filename)
